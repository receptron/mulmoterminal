...
export const planStepSchema = z.object({
  ...
  // claim that it finished is never what decides.
...
export type ComposeResult = { ok: true; steps: PlanStep[] } | { ok: false; problems: string[] };
...
function splice(base: readonly PlanStep[], extra: readonly UsecaseStep[]): PlanStep[] {
  const after = (id: string | undefined): PlanStep[] => extra.filter((step) => step.insertAfter === id).map(withoutAnchor);
  return [...base.flatMap((step) => [step, ...after(step.id)]), ...after(undefined)];
}
