...
> **日付印（2026-08-17）: この項の「2 段」は現存しない。** `deploy` 操作も
> `apps/{aid}/staging/{cid}` も `/staging/{aid}` も撤去され、書き込みは publish の 1 段だけに
> なった（`plans/feat-shared-app-no-staging.md`）。publish 前に動かして確かめる場所は
> `preview` である。**決定として読むところは変えていない** — 「git が真実で Firestore は
> 成果物」「provenance のキーは publish だけが書く」は今も効いている。**書き換えないのは、
> 後の項がこの記述を根拠にしているためである。**
...
> **未実装・保留（2026-08-17 時点）。以下は決定であって、現物ではない。**
> `aidEnv` は宣言に書けるだけで**誰も読んでいない** — `server/backends/sharedApp/declare.ts:379`
> のコメントがそう書いている。`kind: "uuid"` も無い（`common/worktreeEnv.ts` の kind は
> `port` と `slug` の 2 つだけ）。`worktreeEnv` は**値**を配る仕組みで、worktree ごとに新しい
> 識別子を**生成**する能力を持たないので、そのままでは代替にならない。
>
> **踏んだときの症状**: 同じリポジトリの worktree 2 つが 1 つの `aid` を共有する。feature
> ブランチでスキーマや宣言を変えて publish すると、**本番のレコードにそのまま当たる** —
> 「worktree は安全に試すための場所」という期待と真逆で、壊れたことは本番を見ている人にしか
> 見えない。今の回避策は「共有アプリの publish は main の worktree からだけ行う」ことである。
...
> **日付印（2026-08-17）: 入口も操作も、以下の形では現存しない。** 入口は
> `/a/{slug}`（公開）・`/m/{slug}`（ロールを持つ人）・`/p/{slug}`（participant）の 3 つで、
> `/staging/{aid}` は無い。操作は `deploy` / `publish` の 2 段ではなく publish の 1 段で、
> 「招待の前に本物で試す」段は `preview`（著者の手元で実ブラウザ）が担っている。
> **書き換えない** — この計画の「やらないこと」と publish の順序（`public` を最後に書く）は
> D10 のこの記述を根拠にしており、現状に合わせて書き直すと根拠のほうが消える。
...
```text
https://<host>/a/{slug}        公開の顔。未サインインでも読める。slug を確保して初めて生きる
https://<host>/staging/{aid}   名簿の人の入口。aid を直接指す。slug を経由しない
```

> **実装で `/a/{slug}` になった。** ここは当初 `/{slug}` と書いていたが、トップレベルの
> 1 セグメントは既存のページのルートと衝突する。現物は `../mulmoserver`
> `src/router/index.ts` の `a/:slug`（`/staging/:aid` はそのまま）。
