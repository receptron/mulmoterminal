...
cap は開いた瞬間の箱に対してしか正しくないので、**開いている間だけ** 3 つ張って測り直す
（閉じたら全部外す。unmount でも外す）:

- ウィンドウの `resize`
- セルの `ResizeObserver` — セルはウィンドウイベント無しでもサイズが変わる（タイルが 1 枚
  増える、グリッドがページ送りする）
- `scroll`（capture） — スクロールは両方の矩形を動かすがサイズは変えないので、上の 2 つは鳴らない

高さ 0 の箱は「小さい箱」ではなく**測れていない**（teleport 途中のセル、jsdom）。そこに cap を
掛けると `max-height: 0` になってメニューごと消え、cap が防ごうとしている clip より悪いので、
`bottom <= top` は null（= cap しない）に倒す。
jsdom には `ResizeObserver` が無いので、コンポーネント側はそこを skip し、テストは
`vi.stubGlobal` で代役を立てて disconnect まで押さえている。
