...
| **P0c** | **正規化関数の住所を決める（P1 の入口）** — A（core の読みの境界）と B（ページに渡す直前）は**同じ関数**でなければならないが、core は MulmoClaude、sharedapp は receptron で互いに依存が無い。選択肢は 4 つ: ① core に置き sharedapp が依存する ② sharedapp に置き core が依存する ③ 依存中立の小さいパッケージを 1 つ作る ④ 各リポジトリに実装を置き、**同じテストベクタ**で固定する。決めるのは住所と、それぞれの側に要る変更。D1 の未解決の問いはこの行で閉じる | 決める |
