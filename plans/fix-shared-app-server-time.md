...
- **ページ**が欲しいのは「読めて、辞書順が時刻順」。`2026-08-15T01:45:54.605Z` が両方を満たす。
...
1. **形を 1 つにする** — ページも DSL も `…Z`。core の `parseIsoDateTime` を広げ、
   カレンダーは表示時にローカルへ落とす。1 つの値、1 つの形。核心を触るぶん重い。
2. **消費者ごとに 1 つずつ** — ページは `…Z`、DSL は `YYYY-MM-DDTHH:MM:SS`。
...
以後ルールの不変条件（値が動いていないこと）に引っかかって、本人も編集者も**そのレコードを
二度と更新できなくなる**。

- 決めること: 書き込み経路で `stampField` を常に落とすか、復号した値を書き戻せない印にするか。
- 落とす場合、それを知っているのは誰か（`app.json` を読めるのはホスト、`putItems` は
  コレクション側）。ここが一番ややこしいので、実装より先に答えを決めること。
...
| | やること | リポジトリ |
| --- | --- | --- |
| P0 | 実地確認: gym テンプレのアプリで順位が申込順と一致するか。あわせて `parseIsoDateTime` の呼び出し元を数え、D2 の 1 と 2 を決める | MT（+ MulmoClaude を読む） |
...
**1 つのサーバ時刻フィールドについて、ペインの表・手元のプレビュー・本番のページ・
移行ゲートの 4 か所が、同じ 1 つの値を言うか。**
