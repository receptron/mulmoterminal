...
**(6) 表のソートは今のままで通る。** `dateSortValue` は `Date.parse` を使うので、
`…Z` はそのままエポックになる（`sortItems.ts:72`）。
...
- **記録の lint**（`recordZ.ts`）が `datetime` の値として **civil の形か、RFC3339 の絶対時刻か
  のどちらか**を受ける。絶対時刻の判定は `isIsoDateTimeWithOffset` が既にあり、
...
| | やること | リポジトリ |
| --- | --- | --- |
| ~~P0~~ | **済み（3b）** — 4 経路の実測、テンプレのソートが入力順のままであること、`parseIsoDateTime` の消費者 3 つ、D2 の決定。**残: 実アプリで端から端まで**（gym テンプレのアプリに順番を空けて 2 件、`/p/{slug}` の順位を見る。同一ミリ秒に 2 件書いて `nanoseconds` に何が入るかも） | MT |
