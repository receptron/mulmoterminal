...
単一の共有ファイルにすると全インスタンスがそこで競合する。ワークスペースで分ければ通常は各サーバが自分の
ファイルを単独で所有する。

ただし `PORT` は設定可能なので、**同一ワークスペースに 2 台**が立つ構成は否定できない（Codex 指摘）。
そのため sweep の冒頭でディスクの内容を取り込んでから保持判定を行う: 自分が登録していない id も回収対象に
なり、相手に上書きされた自分の id は次の sweep で復帰する。残る窓（取りこぼした id が追跡外になる時間）は
次の sweep で自己修復し、その窓の中でプロセスが死んだ場合のみ恒久化する。ロックファイルは stale lock と
いう発生確率の高い故障モードを持ち込み、compaction 付き journal も同じ競合を持つため、この自己修復窓を
採る判断とした。書き込みは temp + rename の原子的書き込み（`files/atomic-write.ts`、`backends/feeds.ts`
から共通化）で、書き込み中のクラッシュによる登録簿全損も防ぐ。
