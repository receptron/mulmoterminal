...
**登録簿は「1 ファイルのリスト」ではなく「セッション 1 つにつき 1 ファイルのディレクトリ」**にする
（`~/.mulmoterminal/scheduled-sessions/<workspace>/<id>.json`）。リスト形式は 1 件追加するたびに全体を
書き直す必要があり、`PORT` は設定可能なので**同一ワークスペースに 2 台**が立つ構成は否定できない
（Codex 指摘）。その read-modify-write は id を取りこぼし得て、追跡外の id ＝ 誰も回収しない id ＝
リークの再来になる。1 セッション 1 ファイルなら書き手同士が同じファイルに触れないので、**取りこぼす窓が
そもそも存在しない**（共有操作は「自分のファイルを作る」と「期限切れを unlink する」だけ）。ディレクトリ
自体が状態なので、sweep のたびに読み直すだけで他インスタンスの登録も自然に回収対象に入る。

ディレクトリ名はワークスペースパスから作るが、**パスをそのまま使わない**。Windows のパスは `\` と `:` を
含み、そのままではファイル名にできず（書き込み失敗 or 意図しない場所）、Windows だけ再起動時の回収が
静かに死ぬ（Codex 指摘）。英数字以外を `-` に畳んだ上で、畳み込みによる衝突を避けるため実パスの
sha256 先頭 8 桁を付ける。長いパスでも名前が伸びないよう slug は 60 文字で切る。
