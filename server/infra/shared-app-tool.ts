...
export const MANAGE_SHARED_APP: ToolDefinition = {
  ...
  prompt:
    ...
    "It also refuses a declaration with NO `aid` rather than generating one, and that refusal is not a thing to work around: at publish a missing id means the app lost its identity, and minting would publish a SECOND app — new document, roster of one, none of the records, the first one left where it is. Put the original value back (app.json is committed: `git show HEAD:app.json`). Never clear the `aid` to start over, and never delete the app document from the console: Firestore does not cascade, and every record, page and config document under it is authorized THROUGH it, so while it is missing everything below is denied to every rules-bound reader. Publishing again under the SAME aid re-creates it and reaches them once more, which is what makes the aid the thing to protect.",
  parameters: {
    type: "object",
    properties: {
      action: {
