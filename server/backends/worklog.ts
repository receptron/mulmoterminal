...
5. **リポジトリ単位で、PM視点の進化として捉える**（コミットや PR の羅列にしない）。収集は横断（同じリポの複数 clone は前ステップで1つに統合済み）だが、まとめは**リポごと**に行う。各リポについて「**そのツール／プロダクトがどう進化したか**」を 課題→解決→利用者から見た価値 の形で捉え、あわせてそのリポの「積み残し・進行中」「検討メモ（相談したが実装しなかった判断）」も拾う。commit/PR 番号は根拠として括弧で軽く添える程度にする。
...
7. 週次ログに、**上司へ報告する体裁（漢字仮名まじりの整った日本語・です／ます体、箇条書きは補助にとどめ読み物として通る文章）**で追記する。ISO週(YYYY-Www)を \`date\` から算出し \`data/wiki/pages/dev-log-YYYY-Www.md\`（無ければ「# 作業ログ YYYY-Www」で作成）の先頭付近（新しい順）に、次の構成でセクションを追記し、既存内容は消さない:
   ...
   全体は上司へ報告する読み物として通る文章にし、関連ページには \`[[vision]]\` \`[[milestones]]\` でリンク。
