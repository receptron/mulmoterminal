...
- **書き込み対象は次のファイルに限定する**（これ以外は作成・変更・削除しない）: 週次ログ \`data/wiki/pages/dev-log-YYYY-Www.md\` / 一覧ハブ \`data/wiki/pages/worklog.md\` / \`data/wiki/pages/vision.md\` / \`data/wiki/pages/milestones.md\` / 状態 \`config/scheduler/worklog-state.json\`。加えて \`data/wiki/index.md\` は**一覧ハブのエントリを1つ追記する目的でのみ**触れてよい（既存の記載・順序は壊さない。他の変更は禁止）。
...
7. 週次ログに、**上司へ報告する体裁（漢字仮名まじりの整った日本語・です／ます体、箇条書きは補助にとどめ読み物として通る文章）**で追記する。ISO週(YYYY-Www)を \`date\` から算出し \`data/wiki/pages/dev-log-YYYY-Www.md\`（無ければ、先頭に frontmatter〔\`title: 作業ログ YYYY-Www\` と \`tags: [worklog]\`〕を付け「# 作業ログ YYYY-Www」で作成。タグは Wiki の絞り込みで一覧化するため必須）の先頭付近（新しい順）に、次の構成でセクションを追記し、既存内容は消さない:
...
7b. **一覧（ハブ）と索引を維持**（mulmoterminal の Wiki はフラットでサブディレクトリを持てないため、これで"一覧"を実現する）:
   ...
   - \`data/wiki/index.md\` の「## ページ一覧」に、ハブページ（\`[作業ログ 一覧](pages/worklog.md) — 開発作業の週次ログ #worklog\`）のエントリが**まだ無ければ1つだけ**追加する（既存の記載や順序は壊さない）。週次ページ自体は index.md に増やさない（ハブと \`#worklog\` タグからたどれる）。 \`config/scheduler/worklog-state.json\` を \`{"lastRunAt":"<今のISO>"}\` で更新（**書き出しに成功した時のみ**。失敗時は更新せず、次回に取りこぼしを持ち越す）。
