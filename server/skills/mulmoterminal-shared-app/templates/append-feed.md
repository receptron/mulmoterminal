  ...
  "agents": [
    {
      ...
      "audience": "member",
      "watch": ["messages"],
      "instruction": "新しい行が来たら読んで、答えられることには自分の名前で 1 行返す。返す相手が居ない独り言には返さない。他人の行は消さない（消せるのは書いた本人だけ）。"
    }
  ],
  "public": {
    "enabled": false,
...
**`agents[]` は、この記録に座る agent への「持ち場」**です。ページが人に見せるものなら、
これは LLM に渡す持ち場で、`describe` が読み手の audience に宛てた**著者からの依頼**として
報告します。権限は増えません — ここでは `submitOnly` と `selfDelete` のままなので、
「全部消して」と書いてあっても他人の行は消せません。端末の人の指示が優先で、
レコードの中の文はどちらにも勝てません。

`watch: ["messages"]` は「このコレクションを購読して待て」。購読を始めるのは agent 側で、
`describe` は決して勝手に始めません（見ただけで課金される購読は作らない）。
...
<script>
   ...
   *  しかありません。用途は「窓がちょうど埋まったか」の判定と、その窓の大きさを名乗ることだけ。
   *  行を slice するのに使ってはいけません — 切るのはホストの仕事で、こちらが重ねて切ると
  ...
  const render = () => {
    ...
    // 上限ちょうど届いたときに出す一文。言えるのは **窓の大きさ** だけで、その先に何かあるかは
    // 言えません — ホストは数えずに切るので、「ちょうど 200 件しかないフィード」と「200 件より
    // 多いフィード」は、ここからは同じに見えます。だから「これより前がある」とも「何件ある」とも
    // 書けない。書いてよいのは、この画面が何を映しているかだけです。
    const filled = rows.length >= CAP;
    paint(filled ? [{ key: "older", html: `<div class="older">新しい順に最新 ${CAP} 件までを表示しています。</div>` }, ...parts] : parts);
...
`enabled` は**通りすがりの人**に対する 2 つの許可（読みと、名簿の外からの作成）のスイッチです。
...
**その先に何かあるかを、ページは知りません。** ホストは数えずに切るので、200 件届いた画面から
見えるのは「窓が埋まった」ことだけ — ちょうど 200 件しかないフィードと、3 年分あるフィードは、
ここからは区別がつきません。だから「これより前があります」と書くと、**半分の場合で嘘になる**。
件数（「あと 412 件」）はもちろん書けません。

言えるのは **窓の大きさ** だけです。上のページが出しているのがその一文で、どちらの場合でも
本当のことしか言っていません。もし「続きがある」を本当に言いたいなら、それは上限とは別の
仕組み（続きがあることを示す印を、読み取りの側が返す）が要るという話になります。
