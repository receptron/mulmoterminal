...
describe("DEFAULT_BUTTONS", () => {
  it("is the starter set (picker, reveal, in-app files, new terminal, PR) as config buttons", () => {
    expect(DEFAULT_BUTTONS.map((b) => b.id)).toEqual(["pick-file", "reveal", "files", "terminal", "pr"]);
    ...
    // pr self-hides outside a repo (isGitRepo) and without an open PR (resolver).
