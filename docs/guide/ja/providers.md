...
| キー | 意味 |
|---|---|
| `id` | 他の設定から参照する名前。英数と `. _ : / - ~` のみ。**組み込みの 27 モデルは `id` がちょうど `openrouter` のときだけ自動で出ます**——別の id（Moonshot・社内 LiteLLM 等）ではプリセットが付かないので、[モデルを手で列挙](#add-models)してください |
| `label` | 選択画面に出る表示名 |
