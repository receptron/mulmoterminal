...
- **単独で割り当てたキーが優先されます。** 2打の1打目を、単独でも割り当てている（動作、`copy`・`paste`、`send` のどれでも）と、そちらがキーを取り、2打は始まりません。起動時の確認で警告が出ます。例外は、ターミナルを拡大しているとき（またはしていないとき）だけ働く動作です。その動作が働かない側の状態ではキーを譲るので、そこでは2打が始まります。
