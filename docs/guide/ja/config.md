...
- そういうキーは **5 件の枠を消費しません**。**Settings → Toolbar pins** で次にどれかをチェックした時点で
  ファイルからも消えます。つまり昇格させたものをピン留めから外しても損はなく、あとでまた昇格させるのも
  チェック 1 つです。
...
| `toolbarPins` | ツールバーにもボタンを出すピン留め（例 `["collection:works"]`）。最大 5 件、既定は空（→ [毎日開くお気に入りをツールバーに出す](#toolbar-pins)） |
