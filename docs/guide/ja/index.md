...
> **4.22.0 が出ました。ピッカーのエージェントが 2 つ増えています。** **GitHub Copilot CLI** と
> **Cursor CLI** がセルで動くようになり、これで 7 つです。ここで効いてくる問いは「動くかどうか」
> ではなく、**「終わったときに教えてくれるかどうか」** です — 答えがエージェントごとに違うからです。
> **Cursor は Claude に次いで、自分で緑になる 2 つ目**で、見ていないセルには注目マークが付きます。
> **「入力待ち」を出せるのは Claude だけ**です。
> [7 つのどれが何をするか / 新しい 2 つの使い始め方](v4.22.0.html)。
