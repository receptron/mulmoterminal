...
> **4.19.0 が出ました。** コレクションから始めたチャットが、読んでいたものを閉じずに
> **その下のペイン**で動くようになり、コレクション発のセルには**どのコレクションか**が出ます。
> 確かめる価値のある修正が 2 つ: 固まったセルを直すボタンが**セルを殺していた**件と、
> MulmoTerminal が自分の状態を**あなたのプロジェクトに書き込んでいた**件（消しても定期タスクが
> また書くので無駄でした）。どちらも直っています ——
> [入っているかの確かめ方](v4.19.0.html)。
