...
> **4.17.0 が出ました。** ピン留めしたコレクションのうち数件を、**ツールバー自体**に置けるように
> なりました。どの画面からでも一手です（設定が要るのはこれだけ: **Settings → Toolbar pins**）。
> MulmoClaude も使っている方へ: こちらでピン留めしても、共有ファイルに保存された**色が消えなく**
> なりました。stories ルート外のデッキも Canvas で開けます ——
> [設定のしかたと、直したもの](v4.17.0.html)。
