...
> **4.18.0 が出ました。** エージェントが、書いた **3D モデルを自分で見てから**見せられるように
> なりました（設定は不要。ラスタライズにはマシン上のブラウザが要ります）。また、`/clear` した
> 会話が **tmux の無いホストでは再起動のたびに戻ってきて**、次の 1 ターンで再送されていました
> （ある利用者の実測で 478,000 トークン）。直っています ——
> [入っているかの確かめ方](v4.18.0.html)。
