...
| `sessionIdleReapDays` | How long a terminal may sit with **nobody attached and no output** before the server ends it (default 7 days, `0` disables the sweep entirely, 0–365). The conversation is not lost — a transcript on disk resumes without the tmux session; the process and its scrollback are. Editable in Settings → **Sessions that survived a restart**, beside the list it acts on |
| `sessionReapIntervalHours` | How often that sweep runs while the server is up (default 6 hours, `0` means at start only, 0–168). Start-only was the original behaviour and it means a server you never restart never ends anything. A change re-arms the timer at the next start. Editable in the same Settings section |
