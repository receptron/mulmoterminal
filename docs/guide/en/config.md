...
| `sessionReapIntervalHours` | How often that sweep runs **again while the server is up** (default `0`, which is off — it then runs only at start; 0–168). A server you never restart otherwise never looks again. Weaker than the sweep at start: a session this server holds a pty for is skipped whatever its age. `sessionIdleReapDays` is still the threshold, and at `0` this changes nothing. Armed at boot, so a change takes effect at the next start. Editable in the same Settings section |
